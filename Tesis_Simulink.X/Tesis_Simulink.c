...
void Tesis_Simulink_step(void)
{
   ...
   */
  Tesis_Simulink_DW.A = Tesis_Simulink_DW.U1Rx;
   ...
   */
  rtb_Compare_f = ((Tesis_Simulink_DW.U1Rx & 2) >> 1 == 1);

  /* S-Function (MCHP_Digital_Output_Write): '<S5>/Digital Output Write' */
  LATBbits.LATB0 = rtb_Compare_f;
   ...
   */
  rtb_Compare_l = ((Tesis_Simulink_DW.U1Rx & 4) >> 2 == 1);

  /* S-Function (MCHP_Digital_Output_Write): '<S6>/Digital Output Write' */
  LATBbits.LATB1 = rtb_Compare_l;
  ...
  for (i = 1L; i - 1L < 1L; i++) {
    rtb_FIR10 = 0.0F;
    for (j = 0L; j < i; j++) {
      rtb_FIR10 += Tesis_Simulink_DW.DataTypeConversion2 *
        Tesis_Simulink_ConstP.FIR10_Coefficients[j];
    }

    for (j = 0L; j < 11L - i; j++) {
      rtb_FIR10 += Tesis_Simulink_ConstP.FIR10_Coefficients[i + j] *
        Tesis_Simulink_DW.FIR10_states[j];
    }
  }

  /* Update delay line for next frame */
  for (i = 8L; (int16_T)i >= 0; i = (int16_T)i - 1) {
    Tesis_Simulink_DW.FIR10_states[(int32_T)(1 + (int16_T)i)] =
      Tesis_Simulink_DW.FIR10_states[i];
  }

  for (i = 1L; i - 1L < 1L; i++) {
    Tesis_Simulink_DW.FIR10_states[1L - i] =
      Tesis_Simulink_DW.DataTypeConversion2;
  }

  /* End of DiscreteFir: '<Root>/FIR10' */

  /* DiscreteFir: '<Root>/FIR4' */
  /* Consume delay line and beginning of input samples */
  for (i = 1L; i - 1L < 1L; i++) {
    rtb_FIR4 = 0.0F;
    for (j = 0L; j < i; j++) {
      rtb_FIR4 += Tesis_Simulink_DW.DataTypeConversion2 *
        Tesis_Simulink_ConstP.pooled2[j];
    }

    for (j = 0L; j < 26L - i; j++) {
      rtb_FIR4 += Tesis_Simulink_ConstP.pooled2[i + j] *
        Tesis_Simulink_DW.FIR4_states[j];
    }
  }

  /* Update delay line for next frame */
  for (i = 23L; (int16_T)i >= 0; i = (int16_T)i - 1) {
    Tesis_Simulink_DW.FIR4_states[(int32_T)(1 + (int16_T)i)] =
      Tesis_Simulink_DW.FIR4_states[i];
  }

  for (i = 1L; i - 1L < 1L; i++) {
    Tesis_Simulink_DW.FIR4_states[1L - i] =
      Tesis_Simulink_DW.DataTypeConversion2;
  }

  /* End of DiscreteFir: '<Root>/FIR4' */
  ...
  for (i = 1L; i - 1L < 1L; i++) {
    rtb_FIR25 = 0.0F;
    for (j = 0L; j < i; j++) {
      rtb_FIR25 += Tesis_Simulink_DW.DataTypeConversion2 *
        Tesis_Simulink_ConstP.pooled2[j];
    }

    for (j = 0L; j < 26L - i; j++) {
      rtb_FIR25 += Tesis_Simulink_ConstP.pooled2[i + j] *
        Tesis_Simulink_DW.FIR25_states[j];
    }
  }

  /* Update delay line for next frame */
  for (i = 23L; (int16_T)i >= 0; i = (int16_T)i - 1) {
    Tesis_Simulink_DW.FIR25_states[(int32_T)(1 + (int16_T)i)] =
      Tesis_Simulink_DW.FIR25_states[i];
  }

  for (i = 1L; i - 1L < 1L; i++) {
    Tesis_Simulink_DW.FIR25_states[1L - i] =
      Tesis_Simulink_DW.DataTypeConversion2;
  }

  /* End of DiscreteFir: '<Root>/FIR25' */

  /* DiscreteFir: '<Root>/FIR3' */
  /* Consume delay line and beginning of input samples */
  for (i = 1L; i - 1L < 1L; i++) {
    rtb_FIR3 = 0.0F;
    for (j = 0L; j < i; j++) {
      rtb_FIR3 += Tesis_Simulink_DW.DataTypeConversion2 *
        Tesis_Simulink_ConstP.pooled3[j];
    }

    for (j = 0L; j < 41L - i; j++) {
      rtb_FIR3 += Tesis_Simulink_ConstP.pooled3[i + j] *
        Tesis_Simulink_DW.FIR3_states[j];
    }
  }

  /* Update delay line for next frame */
  for (i = 38L; (int16_T)i >= 0; i = (int16_T)i - 1) {
    Tesis_Simulink_DW.FIR3_states[(int32_T)(1 + (int16_T)i)] =
      Tesis_Simulink_DW.FIR3_states[i];
  }

  for (i = 1L; i - 1L < 1L; i++) {
    Tesis_Simulink_DW.FIR3_states[1L - i] =
      Tesis_Simulink_DW.DataTypeConversion2;
  }

  /* End of DiscreteFir: '<Root>/FIR3' */
  ...
  for (i = 1L; i - 1L < 1L; i++) {
    rtb_FIR40 = 0.0F;
    for (j = 0L; j < i; j++) {
      rtb_FIR40 += Tesis_Simulink_DW.DataTypeConversion2 *
        Tesis_Simulink_ConstP.pooled3[j];
    }

    for (j = 0L; j < 41L - i; j++) {
      rtb_FIR40 += Tesis_Simulink_ConstP.pooled3[i + j] *
        Tesis_Simulink_DW.FIR40_states[j];
    }
  }

  /* Update delay line for next frame */
  for (i = 38L; (int16_T)i >= 0; i = (int16_T)i - 1) {
    Tesis_Simulink_DW.FIR40_states[(int32_T)(1 + (int16_T)i)] =
      Tesis_Simulink_DW.FIR40_states[i];
  }

  for (i = 1L; i - 1L < 1L; i++) {
    Tesis_Simulink_DW.FIR40_states[1L - i] =
      Tesis_Simulink_DW.DataTypeConversion2;
  }

  /* End of DiscreteFir: '<Root>/FIR40' */

  /* DiscreteFir: '<Root>/FIR2' */
  /* Consume delay line and beginning of input samples */
  for (i = 1L; i - 1L < 1L; i++) {
    rtb_FIR2 = 0.0F;
    for (j = 0L; j < i; j++) {
      rtb_FIR2 += Tesis_Simulink_DW.DataTypeConversion2 *
        Tesis_Simulink_ConstP.pooled3[j];
    }

    for (j = 0L; j < 41L - i; j++) {
      rtb_FIR2 += Tesis_Simulink_ConstP.pooled3[i + j] *
        Tesis_Simulink_DW.FIR2_states[j];
    }
  }

  /* Update delay line for next frame */
  for (i = 38L; (int16_T)i >= 0; i = (int16_T)i - 1) {
    Tesis_Simulink_DW.FIR2_states[(int32_T)(1 + (int16_T)i)] =
      Tesis_Simulink_DW.FIR2_states[i];
  }

  for (i = 1L; i - 1L < 1L; i++) {
    Tesis_Simulink_DW.FIR2_states[1L - i] =
      Tesis_Simulink_DW.DataTypeConversion2;
  }

  /* End of DiscreteFir: '<Root>/FIR2' */
  ...
  for (i = 1L; i - 1L < 1L; i++) {
    rtb_FIR1 = 0.0F;
    for (j = 0L; j < i; j++) {
      rtb_FIR1 += Tesis_Simulink_DW.DataTypeConversion2 *
        Tesis_Simulink_ConstP.pooled3[j];
    }

    for (j = 0L; j < 41L - i; j++) {
      rtb_FIR1 += Tesis_Simulink_ConstP.pooled3[i + j] *
        Tesis_Simulink_DW.FIR1_states[j];
    }
  }

  /* Update delay line for next frame */
  for (i = 38L; (int16_T)i >= 0; i = (int16_T)i - 1) {
    Tesis_Simulink_DW.FIR1_states[(int32_T)(1 + (int16_T)i)] =
      Tesis_Simulink_DW.FIR1_states[i];
  }

  for (i = 1L; i - 1L < 1L; i++) {
    Tesis_Simulink_DW.FIR1_states[1L - i] =
      Tesis_Simulink_DW.DataTypeConversion2;
  }
  ...
  Tesis_Simulink_DW.DataTypeConversion4 = (uint8_T)(rtb_FIR1 < 0.0F ? (int16_T)
...
void Tesis_Simulink_initialize(void)
{
...

  {
    ...
    for (i = 0; i < 10; i++) {
      Tesis_Simulink_DW.FIR10_states[i] = 0.0F;
    }

    for (i = 0; i < 25; i++) {
      /* InitializeConditions for DiscreteFir: '<Root>/FIR4' */
      Tesis_Simulink_DW.FIR4_states[i] = 0.0F;

      /* InitializeConditions for DiscreteFir: '<Root>/FIR25' */
      Tesis_Simulink_DW.FIR25_states[i] = 0.0F;
    }

    /* End of InitializeConditions for DiscreteFir: '<Root>/FIR10' */
    for (i = 0; i < 40; i++) {
      /* InitializeConditions for DiscreteFir: '<Root>/FIR3' */
      Tesis_Simulink_DW.FIR3_states[i] = 0.0F;

      /* InitializeConditions for DiscreteFir: '<Root>/FIR40' */
      Tesis_Simulink_DW.FIR40_states[i] = 0.0F;

      /* InitializeConditions for DiscreteFir: '<Root>/FIR2' */
      Tesis_Simulink_DW.FIR2_states[i] = 0.0F;

      /* InitializeConditions for DiscreteFir: '<Root>/FIR1' */
      Tesis_Simulink_DW.FIR1_states[i] = 0.0F;
    }
  }
}
